...
public class FlutterBluetoothSerialPlugin
  ...
  // Connections
  /// Contains all active connections. Maps ID of the connection with plugin
  /// data channels.
  private final SparseArray<BluetoothConnectionWrapper> connections =
      new SparseArray<>(2);
    ...
    stateReceiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        ...
        case BluetoothAdapter.ACTION_STATE_CHANGED:
          ...
          stateSink.success(intent.getIntExtra(BluetoothAdapter.EXTRA_STATE,
    ...
    pairingRequestReceiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        ...
        case BluetoothDevice.ACTION_PAIRING_REQUEST:
          ...
          Log.d(TAG, "Pairing request (variant " + pairingVariant +
                         ") incoming from " + device.getAddress());
          switch (pairingVariant) {
          case BluetoothDevice.PAIRING_VARIANT_PIN:
            // Simplest method - 4 digit number
            {
              final BroadcastReceiver.PendingResult broadcastResult =
                  this.goAsync();
              ...
              methodChannel.invokeMethod(
                  "handlePairingRequest", arguments,
                    ...
                    public void success(Object handlerResult) {
                      ...
                      if (handlerResult instanceof String) {
                        try {
                          ...
                          // result.error("bond_error", "Setting passkey for
                          // pairing failed", exceptionToString(ex));
                        }
                      } else {
            ...
            // Note: `BluetoothDevice.PAIRING_VARIANT_PASSKEY` seems to be
            // unsupported anyway... Probably is abandoned. See
          ...
          case BluetoothDevice.PAIRING_VARIANT_PASSKEY_CONFIRMATION:
            // Displayed passkey on the other device should be the same as
            // received here.
          case 3: // case BluetoothDevice.PAIRING_VARIANT_CONSENT: // @TODO ,
                  // Symbol not found?
                  //  The simplest, but much less secure method - just yes or
                  //  no, without any auth. Consent type can use same code as
                  //  passkey confirmation since passed passkey, which is 0 or
                  //  error at the moment, should not be used anyway by common
                  //  code.
          {
            final int pairingKey = intent.getIntExtra(
            ...
            final BroadcastReceiver.PendingResult broadcastResult =
                this.goAsync();
            methodChannel.invokeMethod(
                "handlePairingRequest", arguments, new MethodChannel.Result() {
                  ...
                  public void success(Object handlerResult) {
                    if (handlerResult instanceof Boolean) {
                      try {
                        ...
                        Log.d(TAG, "Trying to set pairing confirmation to " +
                                       confirm + " (key: " + pairingKey + ")");
                        // @WARN `BLUETOOTH_PRIVILEGED` permission required, but
                        // might be unavailable for thrid party apps on newer
                        ...
                        ex.printStackTrace();
                        // @TODO , passing the error
                        // result.error("bond_error", "Auto-confirming pass key
                        // failed", exceptionToString(ex));
                      }
                    } else {
                      Log.d(
                          TAG,
                          "Manual passkey confirmation pairing in progress (key: " +
                              pairingKey + ")");
                      ActivityCompat.startActivity(activity, intent, null);
                    }
                    broadcastResult.finish();
          ...
          case 4: // case BluetoothDevice.PAIRING_VARIANT_DISPLAY_PASSKEY: //
                  ...
                  //  displayed pairing key on the remote device. It looks like
                  //  basic asymmetric cryptography was used.
          case 5: // case BluetoothDevice.PAIRING_VARIANT_DISPLAY_PIN: // @TODO
                  // , Symbol not found?
                  //  Same as previous, but for 4 digit pin.
          {
            ...
            // Note: `BluetoothDevice.PAIRING_VARIANT_OOB_CONSENT` seems to be
            // unsupported for now, at least at master branch of Android. See
            // https://android.googlesource.com/platform/frameworks/base/+/master/core/java/android/bluetooth/BluetoothDevice.java#1559

            // Note: `BluetoothDevice.PAIRING_VARIANT_PIN_16_DIGITS ` seems to
            // be unsupported for now, at least at master branch of Android. See
            // https://android.googlesource.com/platform/frameworks/base/+/master/core/java/android/bluetooth/BluetoothDevice.java#1559
    ...
    discoveryReceiver = new BroadcastReceiver() {
      ...
      public void onReceive(Context context, Intent intent) {
        ...
        case BluetoothDevice.ACTION_FOUND:
          ...
          // !BluetoothClass! final String extraName =
  ...
  @Override
  public void
  onAttachedToEngine(@NonNull FlutterPlugin.FlutterPluginBinding binding) {
    Log.v("FlutterBluetoothSerial", "Attached to engine");
    //        if (true) throw new RuntimeException("FlutterBluetoothSerial
    //        Attached to engine");
    messenger = binding.getBinaryMessenger();

    methodChannel = new MethodChannel(messenger, PLUGIN_NAMESPACE + "/methods");
    methodChannel.setMethodCallHandler(
        new FlutterBluetoothSerialMethodCallHandler());

    EventChannel stateChannel =
        new EventChannel(messenger, PLUGIN_NAMESPACE + "/state");
    ...
    // Discovery
    EventChannel discoveryChannel =
        new EventChannel(messenger, PLUGIN_NAMESPACE + "/discovery");
  ...
  @Override
  public void
  onDetachedFromEngine(@NonNull FlutterPlugin.FlutterPluginBinding binding) {
  ...
  public void onAttachedToActivity(@NonNull ActivityPluginBinding binding) {
    ...
    binding.addActivityResultListener((requestCode, resultCode, data) -> {
      ...
      case REQUEST_ENABLE_BLUETOOTH:
        // @TODO - used underlying value of `Activity.RESULT_CANCELED` since we
        // tend to use `androidx` in which I were not able to find the constant.
      ...
      case REQUEST_DISCOVERABLE_BLUETOOTH:
        pendingResultForActivityResult.success(resultCode == 0 ? -1
                                                               : resultCode);
        return true;
  ...
  private void ensurePermissions(EnsurePermissionsCallback callbacks) {
    ...
    if (!permissionGranted) {
      ActivityCompat.requestPermissions(activity, requestString,
  ...
  private class BluetoothConnectionWrapper extends BluetoothConnection {
    ...
    public BluetoothConnectionWrapper(int id, BluetoothAdapter adapter) {
      ...
      readChannel =
          new EventChannel(messenger, PLUGIN_NAMESPACE + "/read/" + id);
      // If canceled by local, disconnects - in other case, by remote, does
      // nothing True dispose
  ...
  private class FlutterBluetoothSerialMethodCallHandler
    ...
    public void onMethodCall(MethodCall call, Result result) {
      if (bluetoothAdapter == null) {
        if ("isAvailable".equals(call.method)) {
          ...
          result.error("bluetooth_unavailable", "bluetooth is not available",
                       null);
        }
        return;
      }
    ...
    methodCallDispatching:
          ...
          ensurePermissions(granted -> {
            if (!granted) {
              result.error("no_permissions",
                           "Enabling bluetooth requires bluetooth permission",
                           null);
              return;
            }
            ...
            ActivityCompat.startActivityForResult(
                activity, intent, REQUEST_ENABLE_BLUETOOTH, null);
          });
        } else {
          result.success(true);
        }
      ...
      case "getAddress": {
        ...
        if (address.equals("02:00:00:00:00:00")) {
          ...
          do {
            ...
            try {
              // Requires `LOCAL_MAC_ADDRESS` which could be unavailible for
              // third party applications...
              String value = android.provider.Settings.Secure.getString(
                  activeContext.getContentResolver(), "bluetooth_address");
              if (value == null) {
              ...
              // Settings Secure bank failed", exceptionToString(ex));
            }
            ...
            try {
              ...
              mServiceField =
                  bluetoothAdapter.getClass().getDeclaredField("mService");
              ...
              getAddressMethod =
                  bluetoothManagerService.getClass().getMethod("getAddress");
              String value =
                  (String)getAddressMethod.invoke(bluetoothManagerService);
              if (value == null) {
                throw new NullPointerException();
              }
              address = value;
      ...
      case "setName": {
        ...
        try {
          ...
          result.error("invalid_argument",
                       "'name' argument is required to be string", null);
          break;
        }
      ...
      case "getDeviceBondState": {
        if (!call.hasArgument("address")) {
          result.error("invalid_argument", "argument 'address' not found",
                       null);
          break;
        }
      ...
      case "removeDeviceBond": {
        ...
        try {
          ...
          result.error("bond_error", "error while unbonding",
                       exceptionToString(ex));
        }
        break;
      }
        ...
        bondStateBroadcastReceiver = new BroadcastReceiver() {
          ...
          public void onReceive(Context context, Intent intent) {
            ...
            case BluetoothDevice.ACTION_BOND_STATE_CHANGED:
              ...
              final int newBondState = intent.getIntExtra(
              ...
              case BluetoothDevice.BOND_NONE:
                ...
                break;
              default:
                result.error("bond_error", "invalid bond state while bonding",
                             null);
                break;
              }
      ...
      case "pairingRequestHandlingEnable":
        if (FlutterBluetoothSerialPlugin.this.isPairingRequestHandlerSet) {
          result.error("logic_error",
                       "pairing request handling is already enabled", null);
          break;
        }
      ...
      case "requestDiscoverable": {
        ...
        if (call.hasArgument("duration")) {
          try {
            ...
            result.error("invalid_argument",
                         "'duration' argument is required to be integer", null);
            break;
          }
        }
        ...
        pendingResultForActivityResult = result;
        ActivityCompat.startActivityForResult(
            activity, intent, REQUEST_DISCOVERABLE_BLUETOOTH, null);
        break;
      }
      ...
      case "connect": {
        ...
        BluetoothConnectionWrapper connection =
            new BluetoothConnectionWrapper(id, bluetoothAdapter);
      ...
      case "write": {
        ...
        try {
          ...
          result.error(
              "invalid_argument",
              "'id' argument is required to be integer id of connection", null);
          break;
        }
        ...
        if (connection == null) {
          result.error("invalid_argument",
                       "there is no connection with provided id", null);
          break;
        }
        ...
        if (call.hasArgument("string")) {
          ...
          AsyncTask.execute(() -> {
            try {
              ...
              activity.runOnUiThread(
                  ()
                      -> result.error("write_error", ex.getMessage(),
                                      exceptionToString(ex)));
            }
          });
          ...
          result.error("invalid_argument",
                       "there must be 'string' or 'bytes' argument", null);
        }
        break;
      }
